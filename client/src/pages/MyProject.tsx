        ...
        {loading ? (
          ...
          <div className="py-4 min-h-[80vh]">
            <div className=" ml-22 mx-auto flex items-center justify-between px-10 mt-10 mb-10">
              ...
              <button
                onClick={() => Navigate("/")}
                className="bg-gradient-to-r from-blue-700 to-green-500 text-black font-bold rounded-2xl px-6 py-2 flex items-center gap-2 mr-22"
              >
                <PlusIcon />
                Create New
              </button>
            ...
            <div className="flex flex-wrap gap-3.5 ml-25  shadow-blue-500 ">
              {projects.map((projecto) => (
                <div
                  onClick={() => Navigate(`/projects/${projecto.id}`)}
                  key={projecto.id}
                ...
                >
                  ...
                  <div className="p-4 text-white bg-linear-180 from-transparent group-hover:from-blue-950 to-transparent transition-colors flex flex-col flex-1">
                    ...
                    >
                      ...
                      <div className="flex gap-3 text-end">
                        <button
                          onClick={() => Navigate(`/preview/${projecto.id}`)}
                  ...
                  <div onClick={e=> e.stopPropagation()}>
                     <Trash2Icon onClick={() => deleteProject(projecto.id)}  className='absolute top-3 right-3 scale-0 group-hover:scale-100 bg-white p-1.5 size-7 rounded text-red-800 font-bold text-xl cursor-pointer transition-all'/>
                  </div>
                </div>
              ))}
            </div>
