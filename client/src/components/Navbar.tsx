  ...
  <h1 className="text-white text-lg sm:text-3xl font-semibold">
    PR WebBuilder
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={()=> navigate('./auth/signin')} className="px-6 py-2 bg-indigo-600 active:scale-95 hover:bg-indigo-700 transition rounded-md">
               Get started
            </button>
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
              ...
              <Link to='/projects' onClick={() => setMenuOpen(false)}>My Projets</Link>
              <Link to='/community' onClick={() => setMenuOpen(false)}>Community</Link>
              <Link to='/pricing' onClick={() => setMenuOpen(false)}>Pric</Link>
